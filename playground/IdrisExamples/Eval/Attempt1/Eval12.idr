...
eval : (vs, ws, vars : List Nat) -> (env : List (Nat, Nat)) -> (Expr vars) -> (Unzip env vs ws) -> Maybe Nat 
eval vs ws [] env (Num vars n) u = Just n 
-- if the introduced type has a covering function/decision procedure
eval vs ws vars env (Var vars x r) u = case unzip env of 
                                 (vs ** ws ** r2) => lookupVar vs ws x env ?p2 r2 -- unfolded.
eval vs ws vars env (Add vars v1 v2 e1 e2 p) u = case eval vs ws v1 env e1 u of 
                            Just e1' => case eval vs ws v2 env e2 u of 
