...
eval (y::ws) (x::vars) ((x,y)::env) (Var (x::vars) x Here) (ConsUZ u) = lookupVar (x::vars) x ((x,y)::env) Here
eval [] [] [] (Var (x'::[]) x (There p)) NilUZ impossible -- = lookupVar (x'::[]) x [] (There p)
eval (y::ws) (x'::vars) ((x',y)::env) (Var (x'::vars) x (There p)) (ConsUZ u) = lookupVar (x'::vars) x ((x',y)::env) (There p)
