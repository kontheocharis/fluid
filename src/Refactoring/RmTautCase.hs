    ...
    Just t ->
      case termToCase pos t p of
        ...
        Just (Term c@(Case ce cs) _)
          ...
          | otherwise ->
              error $ "unexpected form of case condition: " ++ printVal ce
        Just e ->
          error $ "Returned something other than a case-expr: " ++ printVal e
  where
    pos = Pos (lx as) (ly as)
...
isAppOf g (Term (App t1 _) _) = isAppOf' t1
  where
...
reflArgs :: Term -> Bool
reflArgs (Term (App (Term (App _t t2) _) t3) _) = ra t2 t3
  where
    ra (Term (V v1) _) (Term (V v2) _) = v1 == v2
    ra _ _ = False
reflArgs e = error $ "unexpected form of case condition: " ++ printVal e
