...
outputToDepPair_clauseRHS (Clause patL LNil) = Clause patL (Pair Z VNil)
outputToDepPair_clauseRHS (Clause ((VP natV) : (arg2 : ((VP vecV) : argRes))) (App (Global "vectToList") (V rhsV))) =
  if (vecV == rhsV)
    then Clause ((VP natV) : (arg2 : ((VP vecV) : argRes))) (Pair (V natV) (V vecV))
    else error "TODO: not implemented"
outputToDepPair_clauseRHS (Clause ((SP (VP natV)) : (arg2 : ((LConsP x (VP vecV)) : argRes))) (App (Global "vectToList") (V rhsV))) =
  if (vecV == rhsV)
    then Clause ((SP (VP natV)) : (arg2 : ((LConsP x (VP vecV)) : argRes))) (Pair (V natV) (V vecV))
    else error "TODO: not implemented"
...
addDomRestrictionPf_clauseRHS
  ( Clause
      ((SP (VP (vnl))) : ((SP (VP (vil))) : res))
      (App (App (App (Global "drop2") (V (vnr))) (V (vir))) (arg3))
    ) =
    if and [(vnl == vnr), (vil == vir)]
      then
        ( Clause
            ((SP (VP (vnl))) : ((SP (VP (vil))) : res))
...
removeSigma_clauseLHS (Clause [arg1, arg2, arg3, arg4] term) =
  error "todo" -- I removed the below because function patterns don't exist
  -- Clause ([arg1, arg2, arg3, arg4] ++ [(AppP (AppP (VP (var "minus")) arg1) arg2), ReflP]) term
...
cleanUp_clauseRHS (Clause patL (Hole m)) = Clause patL (Hole m)
