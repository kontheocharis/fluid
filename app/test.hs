...
insertAtAndRelate_terms2 list ((i,elt):res) = 
    let (l,r) = splitAt (i+1) list   in 
        case l of 
            [] -> let addedOne = [elt] ++ r  in    
                    insertAtAndRelate_terms2 addedOne [(j+1, e) | (j,e) <- res ]
            l -> let addedOne = (map (\i -> l!!i) [0..((length l)-2)] ) ++ [tryInsert_appTerm elt (last l)] ++ [elt] ++ r  in    
                    insertAtAndRelate_terms2 addedOne [(j+1, e) | (j,e) <- res ]
...
updataUsecaseInFunc_clRHS ctorNames funcName indPosn dataPosns (App term1 term2) = 
    let appList  = appTermToList (App term1 term2) 
        defRes = (App (updataUsecaseInFunc_clRHS ctorNames funcName indPosn dataPosns term1) 
                      (updataUsecaseInFunc_clRHS ctorNames funcName indPosn dataPosns term2))      in 
        case last appList of 
            Global str -> if (elem str ctorNames) then  
                                listToAppTerm ((Hole (Var ("holeForConstUse_" ++ str ) 0)):appList)     
                          else if str == funcName then                                  --if recursive call, add holes for args, and relate 
                                let addedHoles = (addHolestoAppList appList dataPosns) in    
                                    listToAppTerm  addedHoles                             
                          else 
                                defRes
            term -> defRes          
...
updataUsecaseInFunc_cl ctorNames funcName indPosn dataPosns (Clause lhs rhs) = 
    (Clause (updataUsecaseInFunc_clLHS indPosn dataPosns lhs)  
...
updataUsecaseInFunc_decl ::  [String] ->  String -> Type -> Int -> DeclItem ->  [(String, RefactorArgs)] ->  (DeclItem, [(String, RefactorArgs)])  
...
updataUsecaseInFunc_item :: [String] -> String -> Type -> Int -> Item ->  [(String, RefactorArgs)] ->  (Item, [(String, RefactorArgs)])  
...
updataUsecaseInFunc_item ctorNames datName indexTy indPosn (Decl decl) q = 
    let (refdDecl, newQ) = updataUsecaseInFunc_decl ctorNames datName indexTy indPosn decl q in 
        (Decl refdDecl, newQ ++ q)


updataUsecaseInFunc_items :: [String] -> String -> Type -> Int -> [Item] ->  [(String, RefactorArgs)] ->  ([Item], [(String, RefactorArgs)])  
...
updataUsecaseInFunc_items ctorNames datName indexTy indPosn (item:items) q = 
    let (recItems, recQ) = updataUsecaseInFunc_items ctorNames datName indexTy indPosn items q 
        (refactoredItem, refQ) = updataUsecaseInFunc_item ctorNames datName indexTy indPosn item q in 
...
getCtorName datName ((Data dat):items) = 
    if dataName dat == datName then 
        (map (\ctorItem -> ctorItemName ctorItem) (dataCtors dat)) ++ getCtorName datName items
    else 
        getCtorName datName items
...
updataUsecaseInFunc_ast datName indexTy indPosn (Program items) = 
    let ctorNames = getCtorName datName items 
        (newItems, q) = updataUsecaseInFunc_items ctorNames datName indexTy indPosn items [] in 
        (Program (newItems), q)
...
insertAtAndRelate_sig2 indPosn list ((i,elt):res) = 
    let (l,r) = splitAt (i+1) list  in 
        case r of 
            [] -> l ++ [elt]
            (rfst:rres) -> let addedOne = l ++ [elt] ++  ([(useVarAsInd indPosn (fst elt) rfst )   ])  
                                         ++ rres in    
                                insertAtAndRelate_sig2 indPosn addedOne [(j+1, e) | (j,e) <- res ]
...
useInCtor_ctors datName indexTy indPosn (((CtorItem cName cTy cDatName):ctors), list) = 
    let (inpTY, resTy) =  piTypeToList (cTy) 
        ...
        newCtor = (CtorItem cName 
                  (listToPiType (newInpTy, resTy) )  
                  cDatName) 
        (recCtors, recList) =  useInCtor_ctors datName indexTy indPosn (ctors, list) in 
        (newCtor:recCtors, list ++ ((cName, dataPosns):recList))
...
updateUsecaseInCtor_items datName indexTy indPosn (((Decl decl):items), list) = 
    let (recItems, recList) =   updateUsecaseInCtor_items datName indexTy indPosn (items,list) in 
        ((Decl decl):recItems, recList ++ list)
updateUsecaseInCtor_items datName indexTy indPosn (((Data dat):items), list) =  
    case dat of 
        (DataItem dName dTy ctors) ->  if dName == datName then  
                                            let (recItems, recList) =   updateUsecaseInCtor_items datName indexTy indPosn (items,list) 
                                                (newCtors, newList) = useInCtor_ctors datName indexTy indPosn (ctors, []) in 
                                                ((Data (DataItem dName dTy newCtors)):recItems, list ++ newList ++ recList)                                 
                                        else 
                                            let (recItems, recList) =   updateUsecaseInCtor_items datName indexTy indPosn (items,list) in 
                                                ((Data dat):recItems, recList ++ list)
...
addPatternVarsOfCtorUse_rhsterm datName indexTy indPosn changedCtors (App term1 term2) = 
    let appList = appTermToList  (App term1 term2)  in  
        case last appList  of 
            Global str -> let changedCtorPosns=  findIndices (\pair -> fst pair == str) changedCtors in 
                            if length changedCtorPosns > 0 then 
                                    listToAppTerm (addHolesToAppList appList (snd (changedCtors !! (changedCtorPosns !!0))) ) 
                            else 
                                    (App (addPatternVarsOfCtorUse_rhsterm datName indexTy indPosn changedCtors term1) (addPatternVarsOfCtorUse_rhsterm datName indexTy indPosn changedCtors term2)) 
...
addPatternVarsOfCtorUse_term datName indexTy indPosn changedCtors (App term1 term2) = 
    let appList = appTermToList  (App term1 term2)  in  
        case last appList  of 
            Global str -> let changedCtorPosns=  findIndices (\pair -> fst pair == str) changedCtors in 
                            if length changedCtorPosns > 0 then 
                                    listToAppTerm (addVarToAppList appList (snd (changedCtors !! (changedCtorPosns !!0))) ) 
                            else 
                                    (App term1 term2)  -- no change needed
...
updateCtorUsecase_items datName indexTy indPosn changedCtors ((Decl decl):items) = --todo: check datName not used in function, move on
...
updateUsecaseInCtor_ast datName indexTy indPosn (Program items) = 
    let (newItems, changedCtors) = updateUsecaseInCtor_items datName indexTy indPosn (items, []) in   -- update use of data in ctors
        Program (updateCtorUsecase_items datName indexTy indPosn changedCtors newItems) -- update use of Ctor in other functions                               
...
--todo: run unification to deduce values of holes
