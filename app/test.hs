...
import Refactoring.AddIndex (addIndex_ast, updataUsecaseInFunc_ast, updateUsecaseInCtor_ast)
...
tryRefactor codeStr datName indexTy indPosn =
  let ast = getAST testFileName codeStr
   in if indPosn < 0
        then ast
        else
          let updatedData = addIndex_ast datName indexTy indPosn ast
           in fst
